...
# tasks-axi quotes blocked_by once a task carries 2+ comma-joined blocker ids, the
# same quoting it applies to any comma-containing field. The shim's routed close
# compares that value against its own identity, so it must strip the quoting or it
# spuriously refuses work that genuinely is held behind the decision.
test_shim_resolve_sees_its_edge_among_multiple_blockers() {
  ...
  tasks_in "$home" block sample-multi-implementation --by "$hold" >/dev/null \
    || fail "could not route dependent work behind the captain hold"
  ...
  run_shim "$home" resolve "$id" route --decision-file "$home/multi-route-decision.txt" \
    --routed-to sample-multi-implementation >/dev/null \
    || fail "the shim resolve spuriously failed to see the durable block among 2+ blockers"

  show=$(tasks_in "$home" show "$hold" --full)
  assert_contains "$show" "state: done" "the resolved hold with a co-blocked dependent did not close"

  show=$(tasks_in "$home" show sample-multi-implementation --full)
  assert_contains "$show" "blocked: yes" \
    "the shim resolve cleared the unrelated second blocker instead of only the resolved hold's edge"
  blocked=$(printf '%s\n' "$show" | sed -n 's/^  blocked_by: //p')
  case "$blocked" in
    *sample-multi-other-blocker*) : ;;
    *) fail "the shim resolve removed the unrelated blocker's edge: $blocked" ;;
  esac
  case "$blocked" in
    *"$hold"*) fail "the shim resolve left the resolved hold's edge in place: $blocked" ;;
    *) : ;;
  esac
  pass "the shim's routed close sees its own edge when the dependent task has 2+ blockers"
}
