...
SH
  ...
  fm_write_meta "$d/state/feat-uncooperative.meta" "window=fm:fm-feat-uncooperative" "worktree=$d/wt" "kind=ship" \
    "harness=claude"
  FM_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/fm-busy-event.sh" arm "$d/state" feat-uncooperative)
  "$ROOT/bin/fm-busy-event.sh" apply "$d/state" feat-uncooperative busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
  start=$SECONDS
...
# head must STOP the scan, never fall through onto the older failed row (axi
# status answers another branch here, so attribution can only go through the
# coarse list). Stopping the scan is not the same as binding nothing: this
# branch has exactly one live row and its head is unresolvable in this
# worktree, which is the mid-flight pipeline-worktree shape
# nm_runs_status_for_branch answers with a coarse `running`. The superseded
# failed row stays out of the answer either way, which is the property under
# test; the armed busy event is the pane fallback that would answer if nothing
# bound.
...
EOF
  ...
  assert_contains "$out" "state: working" "the live crew must still read working"
  assert_contains "$out" "source: run-step" "the sole live unresolvable row binds coarsely, not the older failed row"
