...
EOF
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" brief-light-nomistakes-e3 no-registry-proj --mode no-mistakes --light-verify >/dev/null 2>&1
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" brief-light-directpr-e4 direct-proj --mode direct-PR --light-verify >/dev/null 2>&1
