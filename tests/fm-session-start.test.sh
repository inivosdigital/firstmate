...
EOF
  ...
  # chrome-devtools-axi is a firstmate-only tool never present in the base
  # system PATH, so removing the fake makes it deterministically missing
  # regardless of what the test host has installed (unlike node, which some
  # hosts ship at /usr/bin/node).
