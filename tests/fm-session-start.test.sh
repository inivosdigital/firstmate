...
EOF
  ...
  # Uses chrome-devtools-axi rather than node/tmux/gh: those are common system
  # packages that can shadow the fakebin removal via $BASE_PATH's real system
  # directories, making the diagnostic flakily disappear depending on the host.
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # See test_output_ordering_diagnostics_lead: chrome-devtools-axi rather than
  # node avoids a real system package shadowing the forced-missing tool.
  rm -f "$fakebin/chrome-devtools-axi"
