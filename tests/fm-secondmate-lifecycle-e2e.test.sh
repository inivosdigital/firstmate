...
phase_handoff() {
  ...
  # present but too old for the atomic multi-ID `mv` the handoff requires (0.2.2+,
  # the helper's own compatibility gate). The downstream recovery and teardown
  ...
  if ! fm_tasks_axi_compatible; then
    echo "skip: tasks-axi lacks atomic multi-ID mv (0.2.2+); backlog handoff delegates to it"
    return 0
  fi
