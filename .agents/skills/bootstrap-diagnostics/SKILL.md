...
- `FLEET_SYNC: <repo>: recovered: <detail>` - the sync self-healed the clone; no action needed, it is reported only so the self-heal is visible. Either the clone had drifted onto a clean detached HEAD holding no unique commits (re-attached the default branch and fast-forwarded), or the sync hit an orphaned `.git/packed-refs.lock` from a raced landed teardown and cleared it (the lock cleared on its own during a bounded retry, or was removed once provably stale per `FM_FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS`).
