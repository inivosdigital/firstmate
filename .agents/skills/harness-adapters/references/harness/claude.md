...
Verified 2026-07-09 on 2.1.206; the shipped constant was raised to 300000 on 2026-07-11.
`../../../bin/fm-spawn.sh` launches every Claude worker and secondmate with `CLAUDE_CODE_AUTO_COMPACT_WINDOW=300000`, scoped like `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false` above, so separately-launched processes auto-compact around 300k tokens the way the primary already does through its own `.claude/settings.local.json`, which they never inherit.
The variable is read once at process startup and capped at the model's real context window, so it can only lower the effective trigger: `min(300000, window)` is 300000 once the window exceeds it and a no-op below that.
This is not a guarantee for every future model choice - an account or model capped at the plain ~200k window would see no change - but it holds for the window tier crew actually runs on, where `--model sonnet` resolves to the ~1M-context beta.
Live evidence at the original 200000 value, with the mechanism unchanged since: `claude --model sonnet` reported `Auto-compact window: 967k tokens` with no override, and relaunching with `CLAUDE_CODE_AUTO_COMPACT_WINDOW=200000` reported `39.5k/200k tokens (20%)` and `Auto-compact window: 200k tokens`.
The primary's own two-variable form (`CLAUDE_CODE_AUTO_COMPACT_WINDOW=1000000` plus `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=20`) is deliberately not copied here, because that pair assumes a 1M window and would cap a smaller-window model or account to a far more aggressive trigger.
