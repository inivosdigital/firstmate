...
| codex | `--model <model>` | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'` (`max` only on the gpt-5.6 family; gpt-5.4/gpt-5.5 cap at `xhigh`) | Verified on codex-cli 0.147.0 (data/learnings.md 2026-08-10). The installed binary schema contains `model_reasoning_effort` and the active config uses it. `max` was rejected on codex-cli 0.142.1 for every model then available; on 0.147.0 the gpt-5.6 family (sol/terra/luna) accepts it end to end, while gpt-5.4/gpt-5.4-mini/gpt-5.5 still cap at `xhigh`. `minimal` is rejected on every model despite appearing in the binary's strings. |
