...
This repo's own upstream (`kunchenguid/firstmate`, the `upstream` remote after the remote swap) is read-only to this fleet with no merge rights, ever, so firstmate-repo ship tasks land local-only through section 7's local-only path rather than merging a PR here.
The pipeline still auto-opens a PR against that upstream, but it is a non-blocking, purely opportunistic contribution back - never a merge gate here and never waited on - and its diff stays clean only while the upstream-drift gap (`docs/configuration.md`) is small, since a large gap makes the PR carry a lot of unrelated local-only history.
...
Before passing the resolved profile to `fm-spawn.sh`, and again once the task has a brief or a diff, run `bin/fm-risk-tripwire.sh <id>`; a hit floors the model/effort to the safety-critical profile (`opus`/`xhigh`, ultracode `independent-review`) regardless of which rule matched, and flags the task through `fm-ultracode-guard.sh` if the matched rule had not already set `ultracode`.
A hit discovered later, at Validate time against the actual diff, still floors the task in place and is never silently downgraded afterward.
Never dispatch on an unverified model any more than an unverified harness: verify a new non-default model end-to-end on a trivial supervised task, including any sub-agent tool path its harness exposes, before a dispatch rule may name it.
...
- **local-only** has the worker stop with a clean ready branch, then waits for the configured merge authority before firstmate uses the guarded fast-forward merge path, which also best-effort pushes the fast-forwarded default branch to `origin` when one exists (silently skipped for a no-remote project).
...
For a task assigned the trivial tier, run `bin/fm-tier-guard.sh <id>` during Validate, or whenever a heartbeat review touches it, to check whether its diff or elapsed time has outgrown that tier's envelope; any tier also escalates once its diff crosses the script's general heavy-scale ceiling, and a worker's own report of stuck root-cause, a shared-path fix, or a buried tradeoff escalates the same way.
Escalate the model or effort in place without losing the task's branch or context, and never silently de-escalate for the rest of that task's life.
Before advancing an ultracode-flagged task to PR-ready, run `bin/fm-ultracode-guard.sh check <id>`; it refuses until a genuinely separate, independently dispatched task has reviewed the finished diff and its findings were addressed, recorded with `bin/fm-ultracode-guard.sh reviewed <id> <reviewer-task-id>`.
...
Any non-scout, non-secondmate teardown also best-effort syncs and restarts that project's live NAS deployment when one is recorded in `data/nas-deployments.md`, through `bin/fm-nas-deploy-sync.sh`; a project with no recorded deployment is a silent no-op.
...
3. For `check:`, act on the named poll result, including merges, a configured `config/autodeploy-logs` failure, and X-mode events.
...
Work routed to a secondmate belongs in that secondmate home's backlog, while a main-side captain decision or relay reminder that needs durability becomes its own work item; use `tasks-axi hold <id> --reason "<reason>" --kind captain` for a captain-gated thread.
...
Post-swap, `origin` is the captain's fork, so this only ever converges the fleet onto that fork and is a no-op on the primary checkout, which is itself the fleet's source of truth.
It never pulls the read-only `upstream` template; folding upstream-template improvements into local `main` is the separate, deliberately dispatched reconciliation task the upstream-drift note in section 1 refers to, never a self-update.

## 13. Agent-only reference skills

These skills are not captain-invocable; load them only at their precise triggers.

- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints any diagnostic or capability line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `CREW_HARNESS_OVERRIDE:`, `CREW_DISPATCH:`, `FLEET_SYNC:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `TASKS_AXI:`, `NUDGE_SECONDMATES:`, `FMX:`, `SERVICE_FAILED:`, `AUTODEPLOY_FAILED:`, `AUTODEPLOY_INERT:`, or `UPSTREAM_DRIFT:`); silence needs no load.
