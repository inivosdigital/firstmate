...
config/spawn-memory-cap  memory ceiling every launched crewmate, scout, and secondmate runs under, one value (a systemd size, a 1-100 percentage of host RAM, or "off"); LOCAL, gitignored; absent means the built-in per-kind default, never "no cap"; inherited by secondmate homes; a host that cannot provide a scope launches unwrapped rather than refusing, so it never becomes a spawn failure; see docs/configuration.md "Agent memory ceiling" and docs/examples/spawn-memory-cap
