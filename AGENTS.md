...
The canonical schema and per-field semantics, including the optional `use.ultracode`/`use.ultracode_role` profile fields, are owned by `docs/configuration.md` ("Crew dispatch profiles"); read them there before writing or editing the file.
...
`bin/fm-ultracode-guard.sh` tracks this mechanically; this section covers when firstmate flags it and section 7's Validate step covers the PR-ready gate it backs.
