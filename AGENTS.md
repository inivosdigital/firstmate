...
This repo's own upstream (`kunchenguid/firstmate`, the `upstream` remote after the remote swap) is read-only to this fleet with no merge rights, so firstmate-repo ship tasks land local-only instead of through the pipeline and PR path: branch, commit, firstmate reviews the diff, the captain approves, and firstmate fast-forwards local `main`.
The pipeline still opens a non-blocking, purely opportunistic PR against that upstream, never a merge gate and never waited on; its diff stays clean only while the `UPSTREAM_DRIFT:` gap (section 3) is small.
...
config/spawn-memory-cap  memory ceiling every launched crewmate, scout, and secondmate runs under, one value (a systemd size, a 1-100 percentage of host RAM, or "off"); LOCAL, gitignored; absent means the built-in per-kind default, never "no cap"; never applied below a 1 GiB floor, which warns and raises rather than letting a typo or a small host kill every launch; inherited by secondmate homes; a host that cannot provide a scope launches unwrapped rather than refusing, so it never becomes a spawn failure; see docs/configuration.md "Agent memory ceiling" and docs/examples/spawn-memory-cap
...
config/autodeploy-logs  optional poll-based autodeploy status-log paths to watch for failure, one per line, blank lines and full-line "#" comments ignored; LOCAL, gitignored; absent or empty = no-op; each log's last line is read on the watcher's periodic heartbeat sweep and a failure (an "ALERT ..." rollup or a STUCK:/FAILED:/needs-attention line, per bin/fm-autodeploy-lib.sh's fm_autodeploy_line_failed match set) surfaces once as a check wake until it clears, while a missing/unreadable log is skipped silently (bin/fm-watch.sh's autodeploy_scan); the same failure also surfaces as a read-only bootstrap AUTODEPLOY_FAILED diagnostic at every session start (bin/fm-bootstrap.sh's autodeploy_logs_check), so an idle fleet still catches it at the next session even when no watcher swept between sessions; when the log-read timeout mechanism (timeout, gtimeout, or perl) is absent on this host, that same bootstrap check instead prints a one-time AUTODEPLOY_INERT diagnostic so the captain learns alerting is disabled rather than assuming a silently healthy deploy; NOT inherited into secondmate homes, whose same-machine bootstrap and sweep would only re-check the same logs; see docs/examples/autodeploy-logs for a copyable sample
config/tmp-alert-threshold  optional /tmp usage-percent threshold (a single integer 0-100), first non-empty non-comment line; LOCAL, gitignored; absent, empty, or unparseable = no-op; usage is read via `df -P /tmp` on the watcher's periodic heartbeat sweep (bin/fm-watch.sh's tmp_alert_scan), enqueuing a check wake the first heartbeat usage crosses the threshold and staying quiet until it drops back under; the same breach also surfaces as a read-only bootstrap TMP_USAGE_HIGH diagnostic at every session start (bin/fm-bootstrap.sh's tmp_alert_check); a host without `df` on PATH but a configured threshold instead prints a one-time TMP_USAGE_INERT diagnostic; the threshold parse and usage read are the single shared bin/fm-tmp-alert-lib.sh implementation; NOT inherited into secondmate homes, whose same-machine bootstrap and sweep would only re-check the same /tmp; see docs/examples/tmp-alert-threshold for a copyable sample; see docs/configuration.md "/tmp usage watch" for the full mechanism
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  nas-deployments.md hand-maintained pipe-table mapping a project to its live NAS deployment, one row per deployed project: "| <project> | <nas_repo_path> | <pm2_process(es)> |" (comma-separated when more than one pm2 process serves it); read by bin/fm-nas-deploy-sync.sh (section 7); absent or a project missing a row means that project has no known live deployment, not an error
  compose-deployments.md  hand-maintained pipe-table mapping a project to its live systemd + Docker Compose NAS deployment - the compose counterpart to nas-deployments.md, kept as a SEPARATE file so a project is never matched by both scripts; one row per project keyed by project name and absolute checkout path, plus optional compose file, systemd unit, required mountpoint to gate on before deploying, migration command, and health check (the full column format is owned by bin/fm-compose-deploy-sync.sh's header); read by that standalone script, which unlike nas-deployments.md is NOT auto-invoked by teardown; absent or a project missing a row means no known compose deployment, not an error
...
state/               runtime records and signals; gitignored
  ...
  <id>.ultracode     present when the dispatched profile set ultracode=true or a risk-floor tripwire hit flagged it; role=<ultracode_role>, plus one review record per review firstmate recorded through bin/fm-ultracode-guard.sh, each pinned to the diff it covered so a review recorded once cannot cover later commits (section 4; the script's header owns the record format and what the check does and does not establish)
  ...
  .run-progress-<id>  last observed validation-run progress token and when it changed, shared by the watcher and the away-mode daemon so an advancing run, or a finished one awaiting merge, defers its wedge alarm (bin/fm-classify-lib.sh); never touch
  .cursor-park-owner .cursor-park-owner.lock .turnend-cursor-blocks   Cursor stop-hook owner record, publication and commit lock, and bounded repair-nag budget; never touch
  .hash-* .count-* .stale-* .stale-since-* .churn-since-* .paused-* .wedge-escalations-* .writing-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak .tmp-alert-surfaced   watcher internals; never touch
...
The generic effort fallback and its precedence are owned by `harness-adapters`: explicit captain and standing configured effort win; otherwise use low for well-understood explicit work, xhigh for ambiguous investigation or design, intermediate levels proportionally, and `max` only where `harness-adapters`' proportionality and model-acceptance conditions both hold.
...
Treat file or subsystem overlap as a risk signal rather than an automatic reason to wait, and dispatch isolated work when each change can be independently implemented and validated and the selected delivery path can reconcile ordinary rebases or conflicts.
Serialize only for a true semantic dependency, shared mutable external state, incompatible concurrent migration, or another concrete condition that makes independent progress or reconciliation unsafe; same-file editing alone is insufficient, and genuine blockers remain durable.

At most 2 tasks may be simultaneously live (dispatched and in flight) per firstmate home at once, as a supervision-overhead control independent of safety-driven serialization decisions.
Work that is otherwise ready to dispatch but would exceed this limit queues in the backlog and becomes eligible when a live task completes.
The captain may explicitly approve dispatching beyond this cap for named work (see "Captain instruction precedence" section); this cap governs new dispatch decisions only and never authorizes interrupting or tearing down already-live work that happens to exceed the limit.
The cap is scoped per firstmate home: secondmates enforce this independently for their own dispatch and are not counted together with the main home or other secondmates.
...
Re-evaluate queued work after every teardown and heartbeat, dispatching items only when dependencies, time gates, and the concurrency cap (section 7) allow it.
...
For a task dispatched at the trivial (Haiku/low) tier, scaffold with `--light-verify` so the brief cites `verify-trivial` instead of the default verification workflow; the flag is rejected for `--secondmate` briefs.
...
It performs guarded fast-forward updates of firstmate and registered secondmate homes, refreshes instructions, nudges updated live secondmates, and never touches anything under `projects/`.
