...
effort_flag_for_harness() {
  ...
  case "$harness" in
    ...
    codex)
      ...
      # codex-cli 0.147.0, the gpt-5.6 family (sol/terra/luna) accepts max
      # end to end; every other model in the catalog still caps at xhigh, so
      # max is omitted there rather than passing an unsupported value.
      case "$effort" in
...
preserve_relaunch_meta() {
  awk -F= '
    BEGIN {
      split("window endpoint_task_id spawned worktree project harness kind mode yolo tasktmp model effort busy_gen spawn_gen traceparent backend herdr_session herdr_workspace_id herdr_tab_id herdr_pane_id zellij_session zellij_tab_id zellij_pane_id orca_worktree_id terminal cmux_workspace_id cmux_surface_id home projects control_relaunch_tx", keys, " ")
...
# After the wrap, not before it: this clears a TRACEPARENT a prior incarnation
# exported into the PANE SHELL, so it must run as a pane-shell statement. Placed
# ahead of the wrapper it would instead become the wrapper's own command, and the
# ";" would leave the real launch running outside the memory scope entirely.
