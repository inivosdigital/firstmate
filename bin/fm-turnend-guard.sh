...
# the next turn's guard confirms it, so blocking this turn is pure noise. This
# applies to both loop-guard modes; the --claude cooperative path below still
# layers its own Stop-owned auto-arm wait on top for the claim it tracks.
arm_deadline=$(( $(date +%s) + ARM_WAIT ))
while :; do
  if fm_watcher_healthy "$STATE" "$WATCH" "$GRACE" "$FM_HOME" || rearm_in_flight; then
