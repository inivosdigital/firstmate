...
while :; do
  ...
  if [ "$child_done" -eq 0 ] && ! fm_pid_alive "$child"; then
    ...
    rc=$?
    child_done=1
    # The forked watcher already exited before we saw a healthy beacon; this
    # arm is returning its outcome, not bringing a watcher up, so stop
    # declaring a handoff in progress before owned_child_finished decides
    # whether that outcome was a reported wake or an unexpected clean exit.
    arming_clear
    owned_child_finished "$rc"
    exit $?
  fi
