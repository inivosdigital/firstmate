...
#
# fm_autodeploy_line_failed tests a status log's last line for failure. The log
# format is whatever an external per-project autodeploy tool writes (e.g.
# beamanalyzer's deploy job): a healthy run ends in an "ok ..." rollup and a
# failed one in an "ALERT ..." rollup, plus firstmate's own
# STUCK:/FAILED:/needs-attention alert-line format (bin/fm-fleet-sync.sh emits
# the STUCK: variant). The ok/ALERT rollup is the deploy tool's own convention,
# not something bin/fm-fleet-sync.sh produces. Both readers of
# config/autodeploy-logs - the periodic sweep (bin/fm-watch.sh's autodeploy_scan)
# and the session-start bootstrap check (bin/fm-bootstrap.sh's
# autodeploy_logs_check) - source this so the match set has exactly one
# definition.
