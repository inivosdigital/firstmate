...
#     strict fm_nm_head_matches_worktree; bin/fm-crew-state.sh's read-only
#     attribution additionally accepts a live run on a locally-unresolvable
#     head, which is not ownership proof strong enough to abort over) both
...
# Does the given ref's tree already contain everything HEAD introduces? 3-way merges
# the ref (ours) with HEAD (theirs) off their merge base into a throwaway index: when
# HEAD introduces nothing the ref does not already contain (e.g. its change landed via
# squash), the merged tree equals the ref's tree. read-tree + write-tree is used
# instead of `git merge-tree --write-tree` (git >= 2.38 only) so this stays portable to
# older git; a real merge conflict leaves unmerged index entries, write-tree fails, and
# the check stays inconclusive so the caller refuses rather than guesses.
content_matches_ref() {
  local ref=$1 default_tree base merged_tree tmpdir rc
...
NM_TEARDOWN_TIMEOUT=$(fm_sanitize_timeout_bound "${FM_TEARDOWN_NM_TIMEOUT:-10}" 10)
# Guarantees the call is gone by TIMEOUT+KILL_AFTER even if no-mistakes ignores
# or is stuck past the initial signal; see bin/fm-nm-run-lib.sh/fm-classify-lib.sh.
NM_TEARDOWN_KILL_AFTER=$(fm_sanitize_timeout_bound "${FM_TEARDOWN_NM_KILL_AFTER:-2}" 2)
TASK_RUN_ID=
...
task_run_is_own_parked_run() {  # <worktree>
  ...
  out=$(fm_nm_run "$wt" "$NM_TEARDOWN_TIMEOUT" "$NM_TEARDOWN_KILL_AFTER" axi status)
...
conclude_task_no_mistakes_run() {  # <worktree>
  ...
  fm_nm_run_checked "$wt" "$NM_TEARDOWN_TIMEOUT" "$NM_TEARDOWN_KILL_AFTER" axi abort --run "$run_id" >/dev/null 2>&1 || true
  if out=$(fm_nm_run_bounded "$wt" "$NM_TEARDOWN_TIMEOUT" "$NM_TEARDOWN_KILL_AFTER" axi status --run "$run_id" 2>&1); then
...
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.meta" \
  ...
  "$STATE/$ID.kimi-turnend-token" "$STATE/$ID.ultracode" \
  "$STATE/.run-progress-$ID" \
