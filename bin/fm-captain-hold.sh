...
command_verify() {
  ...
  # Only the per-entry durability check below reads the backlog, so an origin
  # that recorded no captain call verifies without a backlog backend at all.
  [ -n "$keys" ] && require_tasks_axi
  if [ -n "$keys" ]; then
    while IFS= read -r entry; do
      [ -n "$entry" ] || continue
      verify_hold_durable "$(resolve_entry "$origin" "$entry")"
