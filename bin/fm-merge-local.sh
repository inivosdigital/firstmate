...
# remote-backed. The push is non-interactive (GIT_TERMINAL_PROMPT=0) and bounded
# by timeout when available, so a credential prompt or stalled connection never
# hangs the merge command.
