...
#                          all is treated as). Past that bound, a declared
...
#                          once per declaration; busy_turn_bound_check owns
#                          that handoff); every other pane goes
...
# production owner (wake-lib, classify-lib, backend.sh, transition-lib, all
# sourced transitively). The Herdr event-wait smoke test consumes this same
# owner without sourcing the entire watcher graph.
...
# restart - unless the crew declared the wait itself, which takes the long pause
# cadence instead (busy_turn_bound_check owns that choice). Set generously above
# any legitimate uninterrupted busy stretch, including long tool calls, builds,
# or test runs.
#
...
# window_busy_state: prints the semantic busy-state contract's verdict word for
# a window (bin/fm-busy-lib.sh). The record-and-tail classifiers used here emit
# busy, idle, or unknown; every other word, including the dead an endpoint probe
# would produce through fm_busy_classify_live, prints unknown, which is how a
# non-idle verdict is treated throughout. Callers that only need "is this pane
# provably working" collapse it to a boolean below, but the busy bound needs the
# word itself: idle is a positive sighting that the pane was between calls,
# while unknown only says the contract could not establish that, and the bound
# treats the two differently (see "What may move this bound's start" below).
...
window_busy_state() {  # <window> <tail40>
  ...
  case "${verdict%% *}" in
    busy|idle) printf '%s' "${verdict%% *}" ;;
    *)         printf 'unknown' ;;
  esac
}

# window_is_busy: 0 (busy) iff the task's harness is PROVABLY working. Only an
# exact busy verdict returns 0: idle and unknown both return 1, so a converted
  ...
  case "$since" in
    ...
    *)
      ...
      if [ "$age" -ge "$STALE_ESCALATE_SECS" ]; then
        ...
        # The third liveness input, after the pane and the run step: a crew still
        # writing files behind a static pane is working, whatever the pane shows.
...
#     contract could not tell, which is not evidence the pane was between calls -
#     so it does not move this start;
#   - state/<task>.turn-ended: touched by the harness turn-end hook when a turn
#     actually completed, and by two harnesses more often than that: Pi at inner
#     turn boundaries inside one busy run, and Codex on any notification event,
#     because the notify wiring fm-spawn installs touches the marker without
#     reading the payload. Both extra cases are still real worker activity, and a
...
# verdict is the one event that starts a new stretch, while busy and unknown
# alike leave the anchor where it was - neither is evidence that the current
# call had not begun. A non-idle poll only stamps the poll and, when the clock
# has stepped backward since the previous one, shifts the anchor by that same
# step, which costs the measured age the time since the previous poll instead of
...
# bound through busy_turn_bound_check, which hands it to the existing
# wedge_timer_check unless the crew declared the wait itself, never anything
# that touches the worker itself.
...
# Apply the busy-pane bound to a window whose bound has already crossed,
# honoring the worker's OWN declared external wait. Prints/queues
...
busy_turn_bound_check() {  # <window> <task> <hash> <since-file> <escalation-file>
  ...
  wedge_timer_check "$win" "$since_file" "not seen idle within the bound" "$escalation_file" "$task"
  return 1
}
...
# declared pause get a SEPARATE recovery chance. Reaching it already proves one
# of the only two admissible cases: an ordinary crew whose agent the gate below
# confirmed DEAD, so no live decision gate is being silenced, or a secondmate,
# whose endpoint liveness this function deliberately never reads and which
# therefore cannot supply that confirmation. Without the mate case a mate's
# captain hold - which has no current-state mapping and so arrives as `none` -
# would be silenced by every caller instead of taking the bounded re-surface
# cadence, and a forgotten hold would rot invisibly. A live or merely
# ambiguously read ordinary crew still leaves it none.
pause_state_class() {  # <window> <task>
  ...
  # Read once past the declared-wait gate, so a mate's stale poll costs one
  # metadata scan and the far more common no-declaration path above costs none.
...
EOF
  ...
  while IFS= read -r w; do
    ...
    # exact idle sighting apart from unknown, which a boolean cannot carry.
    ...
    if [ "$h" = "$prev" ]; then
      ...
      if [ "$n" -ge 2 ] && [ "$busy_now" -ne 0 ]; then
        ...
        # the bound - then route it through busy_turn_bound_check, which hands
        # the crossed bound to the same wedge timer unless the crew declared
        # the wait itself.
        paused_bound=1
      ...
      # only place a pane the contract cannot read (unknown) can be surfaced
      # while its footer keeps moving. It gets the same bound and the same
      # absorber choice as a provably busy one rather than an exemption; a
      # declared pause or captain hold still owns such a pane,
