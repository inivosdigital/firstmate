...
| `fm-ultracode-guard.sh`  | Hold an ultracode-flagged task back from PR-ready until a recorded independent review still covers its current diff (guardrail #3) |
...
| `fm-classify-lib.sh`     | Shared captain-relevant and declared-external-wait wake classification vocabulary, plus durable keyed-decision folds and scans |
