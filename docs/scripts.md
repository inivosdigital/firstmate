...
| `fm-brief.sh`            | Scaffold ship, scout, secondmate-charter, and Herdr-lab briefs, each noting the per-worktree Docker Compose project isolation |
...
| `fm-spawn.sh`            | Spawn crewmates, scouts, `id=repo` batches, and secondmates on the resolved harness and runtime backend, recording a per-worktree Docker Compose project name (`compose_project=`) for ship/scout tasks |
...
| `fm-lock-lib.sh`         | Shared "is this git lock provably abandoned?" proof used by teardown, fleet-sync, and NAS deploy sync |
