...
Every new task also records `spawned=<epoch>`, written once when its metadata is created and carried forward unchanged by later rewrites of that file, so supervision has a start for a task that has not completed a turn yet whatever its harness arms.
...
3. **Live task harness scratch, and every home's own top-level session.** Any live task's harness scratch directory, same liveness source, PLUS one synthesized entry per firstmate home (the primary checkout and each secondmate) for that home's own worktree - neither the primary checkout nor a secondmate's own top-level instance is ever spawned by `bin/fm-spawn.sh`, so neither ever gets a `state/*.meta` recording its own worktree, and without this synthesized entry the session most worth protecting would be live only by writing inside the age window (`bin/fm-tmp-lib.sh`'s `fm_tmp_live_tasks`). The Claude Code sanitizer (every non-alphanumeric byte becomes `-`, applied to the worktree's absolute path) was verified empirically: a live session's own recorded scratch path was compared directly against its `state/*.meta` `worktree=` value run through the same transform, and cross-checked against every other live meta record on the machine. That sanitizer is lossy by construction (inherited from replicating Claude Code's own real naming scheme, not something this sweep could change while staying compatible): `/a/foo-bar` and `/a/foo/bar` both sanitize to `-a-foo-bar`, so two structurally different worktree paths can in principle collide onto the same protected name. Collisions among real treehouse pool paths are highly unlikely in practice, and a collision only ever makes the sweep protect MORE than it strictly needs to, never less. **Residual risk, not covered by liveness at all:** a Claude Code session this machine is running that firstmate did not itself spawn - a no-mistakes pipeline worktree, or an ad-hoc session the captain runs directly - has no `state/*.meta` anywhere firstmate can read, under any home, so it is protected only by the age threshold, identically to the gap this same check closes for firstmate's own sessions. Extending protection to those would require firstmate to enumerate another tool's own live-session bookkeeping, which does not exist in any form firstmate can read today; the tradeoff is deliberately left as documented residual risk rather than a naming-convention guess that could silently stop matching the moment that other tool changes its own layout.
...
Every action, removed, would-remove, or skipped, along with its reason, is both printed and appended to `state/tmp-sweep.log` (path overridable via `FM_TMP_SWEEP_LOG`, primarily for testing), one tab-separated line per action, trimmed to the most recent 5000 lines via a unique per-invocation temp file (never a fixed name) so two overlapping invocations rotating at once cannot crash each other or wipe the log, only at worst lose the other invocation's newest few lines to a last-write-wins race.
...
The threshold twin of [Critical service watch](#critical-service-watch-configcritical-services), same shape: `config/tmp-alert-threshold` (local, gitignored) holds a single integer 0-100, the `/tmp` usage percent (via `df -P /tmp`) at or above which firstmate warns.
...
An absent or empty file, or a threshold line that fails to parse as an integer 0-100, is a no-op: no default threshold ships, matching the opt-in pattern of `config/critical-services`/`config/autodeploy-logs`, so this alert is silent and inert until the captain deliberately configures a threshold.
Say so plainly because the incident that motivated this whole feature ([Why this exists](#why-this-exists)) arrived with no warning of any kind - installing the sweep alone does not, by itself, give any advance warning before it starts removing content; only a configured threshold does.
...
A sweep is bounded end to end rather than only at its fetch, records the outcome durably under `state/upstream-sync/`, and queues at most one reconciliation request.
The fetch is not the only step that can wait forever, so one budget caps the whole measured operation (fetch, measurement, backlog read) while each step keeps its own smaller cap, and whichever is tighter wins.
Notifying firstmate is bounded outside that budget so a sweep that ran out of time can still say so, and the sweep's own lock is released on every exit path, including a signal, so one stalled run never locks out every later one.
A host whose `timeout(1)` cannot escalate an ignored `SIGTERM` to `SIGKILL`, or that has no such tool at all, gets the script's own escalating bound rather than an unenforced one, and a bound or tool selection the script cannot honor refuses at the start of the run instead of being reported as a no-change success.
...
Intake de-duplication reads the existing owners rather than keeping a second copy of the task's state: an upstream head this routine already reported, a still-unacknowledged intake note it actually announced, or a filed and unheld backlog item under the reconciliation id (`upstream-drift-alert` by default) each suppress a new request.
Announced is the load-bearing word there: a note the inbox saved but could not wake firstmate for is an unfinished publication rather than a delivered request, so it is recorded as still unannounced and re-announced through the inbox at the next sweep instead of suppressing the request it was supposed to carry.
...
A job is matched by ownership, not by resemblance: the schedule fields and any inline `VAR=value` prefix are skipped, the command word's basename must be exactly the managed or legacy script, and that command must then resolve to this same installation and operational home before this run may replace it.
So one user's crontab can carry a second firstmate home's sweep or legacy drift job and this run leaves it untouched, each home's own entry being labeled with a marker naming the home it was installed for.
An entry naming one of those scripts that cannot be bound either way - the file is gone, unreadable, or never says which home it serves - is preserved and reported as something to check by hand rather than removed on a guess, as is a backup job or environment assignment that merely mentions one of those names.
...
The line also carries every operational selection the script cannot re-derive from its own location - the home, its state and private-records directories, the reconciliation id, and any non-default bound - as inline assignments scoped to that one job, because cron hands a job almost no environment and a dropped home would sweep one fork while still logging into another.
Every one of those values has to be something cron can represent literally, so a path carrying whitespace, a single quote, or a `%` refuses the install instead of writing a line cron would mangle.
...
`ultracode` must be a JSON boolean and `ultracode_role`, when present, a non-empty string, on a rule's `use` profile(s) and on `default` alike; bootstrap validation rejects a present-but-wrong-type value at either location outright rather than silently dropping it, and `bin/fm-dispatch-select.sh` does the same for whatever profile it resolves - in practice a rule's `use` profile(s), since `default` is applied directly and never routed through the selector - because a rule or default that looks ultracode-flagged but resolves as inert would skip the independent-review guard without anyone noticing.
...
That configuration is the whole opt-in: an unconfigured home cannot start the relay and cannot run `fm-inbox.sh say` or `ask`, while `note`, `wake`, `status`, `list` and `drain` need no configuration at all because they make no model call.
...
FM_CREW_STATE_NM_TIMEOUT=10   # seconds allowed per no-mistakes query inside fm-crew-state.sh before its initial signal
FM_CREW_STATE_NM_KILL_AFTER=2   # grace seconds after that signal before a still-running no-mistakes query is force-killed (SIGKILL), not just asked to stop
FM_TEARDOWN_NM_TIMEOUT=10    # seconds allowed per no-mistakes query or abort inside fm-teardown.sh before its initial signal
FM_TEARDOWN_NM_KILL_AFTER=2   # grace seconds after that signal before a still-running no-mistakes query or abort inside fm-teardown.sh is force-killed (SIGKILL), not just asked to stop
...
FM_CREW_ABSORB_TIMEOUT=45   # seconds allowed for the ENTIRE fm-crew-state.sh read inside crew_absorb_class before its initial signal - defense in depth around FM_CREW_STATE_NM_TIMEOUT, bounding the watcher's own liveness beacon against a hung read regardless of cause
FM_CREW_ABSORB_KILL_AFTER=5   # grace seconds after that signal before a still-running fm-crew-state.sh read is force-killed
...
FM_BUSY_TURN_MAX_SECS=14400        # how long a pane no poll has seen idle - busy, or unreadable (unknown) - may go with no proof its current call had not yet started before the same wedge escalation used for a provably-working non-busy stale takes over; inspection-only, never an automatic interrupt or restart; a declared external wait or verified captain-held transfer takes the FM_PAUSE_RESURFACE_SECS recheck below instead; measured from the newest proof available, each of which is either something a poll positively observed about the pane or something the worker itself demonstrably finished, with the task's creation epoch as the floor beneath them, so a worker that lands nothing and ends no turn is measured from whatever it last got done; bin/fm-watch.sh's busy_turn_over_age block owns the exact proof set, the records it refuses, and why a pane with no proof at all is treated as over the bound, and its BUSY_TURN_MAX_SECS comment owns the sizing tradeoff
...
FM_TIER_HEAVY_MIN_LINES=400        # fm-tier-guard.sh: changed-line count that escalates any tier via the general heavy-scale ceiling
...
FM_NAS_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=1 # seconds fm-nas-deploy-sync.sh waits before each of those retries
FM_NAS_SYNC_PACKED_REFS_LOCK_AGE_SECS=30       # min mtime age before fm-nas-deploy-sync.sh treats a leftover packed-refs.lock as provably stale
FM_COMPOSE_DEPLOYMENTS_OVERRIDE=   # alternate data/compose-deployments.md path for fm-compose-deploy-sync.sh, mainly for tests
FM_COMPOSE_PROC_MOUNTS=   # alternate mounts table for fm-compose-deploy-sync.sh's mount gate (and used in place of mountpoint(1) when set); mainly for tests
FM_COMPOSE_SYNC_TIMEOUT=15   # seconds allowed per filesystem/git touch of fm-compose-deploy-sync.sh's compose checkout before it is killed, so an unreachable NAS mount cannot hang the caller
FM_COMPOSE_DEPLOY_TIMEOUT=1200   # seconds allowed per fm-compose-deploy-sync.sh migrate / image build / bring-up step, which are legitimately slow
FM_COMPOSE_HEALTH_TIMEOUT=15   # seconds allowed per fm-compose-deploy-sync.sh post-deploy health-check attempt
FM_COMPOSE_HEALTH_RETRIES=5   # fm-compose-deploy-sync.sh health-check attempts before it reports the deploy as needing attention (a container/app needs a moment after restart)
FM_COMPOSE_HEALTH_RETRY_WAIT_SECS=3   # seconds fm-compose-deploy-sync.sh waits between health-check attempts
FM_COMPOSE_SYNC_PACKED_REFS_LOCK_RETRIES=3        # fetch retries after fm-compose-deploy-sync.sh hits the orphaned .git/packed-refs.lock signature on the shared compose checkout
FM_COMPOSE_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=1 # seconds fm-compose-deploy-sync.sh waits before each of those retries
FM_COMPOSE_SYNC_PACKED_REFS_LOCK_AGE_SECS=30       # min mtime age before fm-compose-deploy-sync.sh treats a leftover packed-refs.lock as provably stale
...
The shared staleness proof lives in `bin/fm-lock-lib.sh`, which `fm-teardown.sh`, `fm-fleet-sync.sh`, `fm-nas-deploy-sync.sh`, and `fm-compose-deploy-sync.sh` all use.
