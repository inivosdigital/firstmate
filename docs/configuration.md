...
FM_BUSY_TURN_MAX_SECS=3600         # how long a busy pane may go with no proof its current call had not yet started before the same wedge escalation used for a provably-working non-busy stale takes over; the only proofs are the newest poll that saw that pane not busy (state/.last-idle-<key>), the latest state/<id>.turn-ended marker, and the spawn epoch in the task's busy-contract gen token, so an idle stretch a poll actually observed is not counted while an unobserved one is, unwatched time never restarts the clock, and a busy pane with no proof at all is treated as over the bound; inspection-only, never an automatic interrupt or restart
