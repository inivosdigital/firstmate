...
`use.ultracode` and `use.ultracode_role` are optional; a profile with `ultracode: true` requires a genuinely independent second pass on its finished diff before PR-ready, and `ultracode_role` labels what that pass should do (`independent-review` is the only role this fleet's own rules use today) - `bin/fm-ultracode-guard.sh` tracks the requirement mechanically, per `AGENTS.md` section 4.
