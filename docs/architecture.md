...
Past the bound the pane is routed through the same wedge escalation, with the identical reason, escalation count, worktree-write deferral, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
...
[`bin/fm-nm-run-lib.sh`](../bin/fm-nm-run-lib.sh)'s header owns the shared head-relation rules, and the script's own header owns the narrower dispensation that lets a live run on a locally-unresolvable head bind.
