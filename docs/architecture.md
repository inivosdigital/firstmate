...
The script's own header owns the exact rule, the alternatives rejected, and the two residual misses it accepts: a risky surface mentioned only inside a prohibition that concedes no work on it, and a work sentence that follows an all-prohibitive opening sentence in the same block, both of which now reach the binding changed-path checkpoint rather than the pre-spawn one.
