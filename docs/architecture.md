...
`crew_absorb_class` (`bin/fm-classify-lib.sh`) carves one exception into that run-step precedence: when the run-step reads `working` for an EXITED crewmate (its backend agent process confirmed `dead`, e.g. an orphaned in-progress CI poll a no-mistakes run never cancelled) and the crew's own last status line declares `paused:`, the declared pause wins, since a dead crewmate cannot resume the run that would otherwise justify overriding it; a live or unconfirmed (`alive`/`unknown`) crewmate still keeps run-step precedence exactly as before.
