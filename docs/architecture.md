...
Exactly three things count as that proof: the newest poll at which a watcher of this home saw that pane not busy (recorded per window in `state/.last-idle-<key>`), the task's latest `state/<id>.turn-ended` marker, and the spawn epoch inside the task's busy-contract gen token.
Everything else is treated as no evidence and never moves the start forward, because a missed wedge is severe while a false alarm costs one supervision look: an absent or malformed record, a poll the watcher never made (its own exit and re-arm, downtime, a capture failure, a fresh watcher meeting a pane already busy), and an epoch left in the future by a wall clock that stepped backward all keep the older start, and a busy pane with no proof at all is treated as over the bound rather than under it.
Two consequences follow and are not hidden: an idle stretch is only uncounted when a poll actually observed it, so an idle-then-busy transition that fell entirely between two polls leaves the new call carrying the previous one's age and can escalate early; and a task whose harness does not arm the busy contract (Grok, Kimi, Codex, Herdr, Muse) has no spawn epoch, so if its first call starts before the watcher's first poll of that window it can escalate once before its first turn completes.
