...
It drops a section whose heading declares a non-change scope, a list item or paragraph whose first sentence is a prohibition, and any remaining sentence that is itself a prohibition; headings are always scanned, a prohibition that concedes adjacent work on the surface it names is never dropped, a brief with no parseable `# Task` section is scanned whole and unnarrowed, and a narrowing that would leave nothing to scan is discarded in favour of the full task body.
The script's own header owns the exact rule, the alternatives rejected, and the residual miss it accepts: a risky surface mentioned only inside a prohibition now reaches the binding changed-path checkpoint rather than the pre-spawn one.
