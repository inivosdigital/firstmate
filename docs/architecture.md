...
The same bound covers every poll that did not see the pane idle, so a pane the busy-state contract cannot read (`unknown`) or whose agent is gone (`dead`) is bounded exactly like a provably busy one; that is what surfaces a wedged worker whose footer keeps ticking, because a pane that keeps changing never reaches ordinary stale detection.
Exactly three things count as that proof: the newest poll at which a watcher of this home got an exact `idle` verdict for that pane (recorded per window in `state/.last-idle-<key>`), the task's latest `state/<id>.turn-ended` marker, and the spawn epoch inside the task's busy-contract gen token.
Everything else is treated as no evidence and never moves the start forward, because a missed wedge is severe while a false alarm costs one supervision look: an absent, malformed or internally inconsistent record (its anchor later than the poll that wrote it), an `unknown` or `dead` verdict, a poll the watcher never made (its own exit and re-arm, downtime, a capture failure, a fresh watcher meeting a pane already busy), and an epoch left in the future by a wall clock that stepped backward all keep the older start, and a pane with no proof at all is treated as over the bound rather than under it.
...
And a task whose harness does not arm the busy contract (Grok, Kimi, Codex, Herdr, Muse) has no spawn epoch, so until a poll sees it idle or its first turn completes it has no trustworthy start at all: while that holds the alarm repeats every `FM_STALE_ESCALATE_SECS` and reaches `demand-deep-inspection` like any other repeat rather than costing a single look, and for Kimi and Codex, which the contract cannot read at all, no poll can supply the missing idle sighting.
