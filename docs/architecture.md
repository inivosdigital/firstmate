...
It drops a section whose heading declares a non-change scope, a list item or paragraph whose first clause is a prohibition, and any remaining clause that is itself a prohibition; headings are always scanned.
Judging clauses rather than whole sentences is what keeps a compound "do this, but do not assume that" instruction on the trip side, and it is the one narrowing bound that carries real weight: across the 605 briefs it was measured on, it keeps a clause in 1597 sentences that read as prohibitions taken whole.
...
The script's own header owns the exact rule, the alternatives rejected, and the residual miss it accepts: a risky surface mentioned only inside a prohibition that concedes no work on it now reaches the binding changed-path checkpoint rather than the pre-spawn one.
