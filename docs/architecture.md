...
`bin/fm-watch.sh`'s `busy_turn_over_age` block is the single owner of which records count as that proof, which lookalike timestamps are deliberately refused, and why anything unreadable keeps the older start rather than moving it.
The shape of the set is worth knowing at this level, because it decides what a healthy long-running worker has to do to stay quiet: every proof is either something a poll positively observed about the pane or something the worker itself demonstrably finished, and underneath both sits the epoch the task was created at.
That last one is a floor rather than a reprieve, and it is why a task on a harness that arms nothing (Grok, Kimi, Codex, Herdr, Muse) does not alarm from launch onwards: a task that has not started talking yet is not a task that wedged, and one genuinely wedged from the moment it launched still alarms one bound after it was created.
Both kinds are needed, because a worker that stays busy from the moment it launched neither goes idle for a poll to see nor ends a turn; without a proof it can refresh while it is still working, its bound measures time since the task began rather than time since its current call began, and is crossed permanently partway through any long task.
