...
A busy pane is otherwise exempt from staleness, but only until `FM_BUSY_TURN_MAX_SECS` has passed since the newest positive proof that its current call had not yet started.
The same bound covers every poll that did not see the pane idle, so a pane the busy-state contract cannot read (`unknown`) is bounded exactly like a provably busy one; that is what surfaces a wedged worker whose footer keeps ticking, because a pane that keeps changing never reaches ordinary stale detection.
Exactly four things count as that proof: the newest poll at which a watcher of this home got an exact `idle` verdict for that pane (recorded per window in `state/.last-idle-<key>`), the task's latest `state/<id>.turn-ended` marker, the spawn epoch inside the task's busy-contract gen token, and the `spawned=` field `bin/fm-spawn.sh` writes once into `state/<id>.meta`.
Everything else is treated as no evidence and never moves the start forward, because a missed wedge is severe while a false alarm costs one supervision look: an absent, malformed or internally inconsistent record (its anchor later than the poll that wrote it), an `unknown` verdict, a poll the watcher never made (its own exit and re-arm, downtime, a capture failure, a fresh watcher meeting a pane already busy), and an epoch left in the future by a wall clock that stepped backward all keep the older start, and a pane with no proof at all is treated as over the bound rather than under it.
Two consequences follow and are not hidden.
An idle stretch is only uncounted when a poll actually observed it, so an idle-then-busy transition that fell entirely between two polls leaves the new call carrying the previous one's age and can escalate early.
And a record carrying none of the four - metadata written before `spawned=` existed, for a harness that arms no gen token and has completed no turn - has no start at all, so it is treated as over the bound and re-alarms every `FM_STALE_ESCALATE_SECS` until some proof exists.
A task whose harness does not arm the busy contract (Grok, Kimi, Codex, Herdr, Muse) is the reason the `spawned=` field exists: until a poll sees such a task idle or its first turn completes that field is the whole of its start, and for Kimi and Codex, which the contract cannot read at all, no poll can supply the missing idle sighting.
It is what keeps those tasks from alarming every `FM_STALE_ESCALATE_SECS` from launch onwards, because a task that has not started talking yet is not a task that wedged and its creation time is the evidence separating the two.
It is a floor rather than a reprieve, so a task genuinely wedged from the moment it launched still alarms one bound after it was created.
`state/<id>.meta`'s mtime is deliberately not a fallback because `bin/fm-pr-check.sh` and `bin/fm-x-lib.sh` rewrite that file during a task's life, and the semantic busy-state record's timestamp is deliberately not the anchor because an adapter rewrites it inside a running call (OpenCode on each `session.status` retry, Pi on each `agent_start`); a start that advances on either would let a hung call defer its own bound for as long as the writes continued.
The `spawned=` field in that same file is content rather than a filesystem timestamp, written once and preserved by both rewriters, so nothing can advance it while a call runs.
That holds only while the field has exactly one value, because a rewriter that preserves the lines it does not own carries an appended second one forward untouched.
So the bound counts a duplicated field as no proof at all rather than reading either line, and `bin/fm-spawn.sh` leaves the field out of a record it cannot resolve to a single readable epoch rather than settling that record on one of them.
Past the bound the pane is routed through the same wedge escalation, with the identical reason, escalation count, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
...
A crew that handed off to a background validation run is idle by design while it waits to be notified, so pane idleness alone is not evidence of a wedge for it, and the same holds for a crew whose run already succeeded and is waiting for its pull request to land, because the merge poll is what watches for that landing.
Before either supervisor escalates such a pane, `crew_run_progress_defers_wedge` (`bin/fm-classify-lib.sh`, the single owner both the watcher and the away-mode daemon call) compares a run progress token from `bin/fm-crew-state.sh --run-progress` against the last one recorded in `state/.run-progress-<id>`.
That token is prefixed with the same reconciled phase the reader would otherwise report, so one read separates the two phases allowed to defer, an advancing run and a finished one awaiting merge, from a run parked at a gate or a failed one, which never do.
A token that changed means the run structurally advanced, so the alarm is deferred and the timer restarts; a token unchanged for `FM_RUN_WEDGE_SECS` means the pipeline has genuinely stopped moving and still escalates, re-stamped so it repeats once per window rather than falling silent.
The rest of the token deliberately carries only the run id, run head, and per-step `name:status` pairs, never elapsed durations, so only structural progress can renew it; a crew with no attributed run or an unreadable token escalates on the ordinary `FM_STALE_ESCALATE_SECS` pane timer exactly as before.
A confidently dead agent also escalates on that ordinary timer while a run is still supposed to be advancing, but not once the run has finished, since a worker that reported its result and exited is in its expected end state rather than wedged.
...
Independent of the crew fleet-scan, every heartbeat also sweeps the optional local `config/autodeploy-logs` list (`bin/fm-watch.sh`'s `autodeploy_scan`): each configured status log's last line is read against the autodeploy failure convention (`bin/fm-autodeploy-lib.sh`'s `fm_autodeploy_line_failed`, shared with `bin/fm-bootstrap.sh`'s session-start counterpart check), a new failure enqueues a `check:` wake and is marked surfaced so it does not repeat, and a later healthy line clears that marker so a recurrence re-arms.
...
`crew_absorb_class` (`bin/fm-classify-lib.sh`) carves two exceptions into that run-step precedence, both gated behind the crew's own last status line declaring `paused:`: a still-open captain-decision hold (`status_open_decisions`) proves the crew cannot progress until firstmate or the captain acts, so the declared pause wins outright regardless of backend liveness (a run genuinely parked at a gate can otherwise read as a plain `working` run-step, since the cross-branch run lookup's coarse fallback carries no per-step gate detail); failing that, an EXITED crewmate (its backend agent process confirmed `dead`, e.g. an orphaned in-progress CI poll a no-mistakes run never cancelled) also licenses the pause, since a dead crewmate cannot resume the run that would otherwise justify overriding it. A live crewmate with no open decision still keeps run-step precedence exactly as before.
Every no-mistakes call inside that read, and the whole read itself as watched from `crew_absorb_class`, is force-killed on a hard timeout (`FM_CREW_STATE_NM_TIMEOUT`/`FM_CREW_STATE_NM_KILL_AFTER`, `FM_CREW_ABSORB_TIMEOUT`/`FM_CREW_ABSORB_KILL_AFTER`) rather than merely signaled.
A plain `timeout N cmd` with no kill-after is only advisory once N elapses, and an uncooperative no-mistakes CLI defeating that soft bound under concurrent validation load caused the watcher's liveness beacon to stall for 89 minutes on 2026-07-09 before this fix landed.
A timed-out read reads as `none` (not provably working), never a false `working`, so it surfaces rather than being silently absorbed.
...
The dispatch file is intentionally judgment-based: firstmate reads the natural-language rules at intake, chooses the best matching rule, and passes only concrete `--harness`, `--model`, and `--effort` axes to `fm-spawn.sh`.
`bin/fm-dispatch-select.sh` owns `quota-balanced` array selection determinism and degrades to the first array element whenever quota data is unusable.
The shell scripts validate the JSON shape and verified harness/effort combinations, but they do not parse task intent or match natural-language rules.
...
The brief scan reads the prose that describes the work, not the prose that fences the work off, because firstmate writes both into the same `# Task` region and a scan that counted the fencing prose fired on two thirds of all briefs.
It drops a section whose heading declares a non-change scope, a prose list item or paragraph whose opening sentence is a prohibition in every one of its clauses, and any remaining clause that is itself a prohibition; headings are always scanned.
The whole-block drop is deliberately confined to prose and to an all-prohibitive opening: a fenced code block is exempt from it, so a caveat comment in any syntax stops taking the snippet under it down, and one work clause anywhere in the opening sentence is enough to keep the block.
Judging clauses rather than whole sentences is what keeps a compound "do this, but do not assume that" instruction on the trip side, and it is the one narrowing bound that carries real weight: across the 613 briefs it was measured on, it keeps a clause in 1661 sentences that read as prohibitions taken whole.
The remaining bounds are narrower than they look and the script's header records each one's measured reach rather than asserting a safety margin.
The script's own header owns the exact rule, the alternatives rejected, and the three residual misses it accepts: a risky surface mentioned only inside a prohibition that concedes no work on it, a work instruction whose verb is not on the closed list that rescues later sentences from a dropped block, and a brief whose narrowing previously emptied and now does not, which switches off the fallback that would have scanned its whole body.
All three reach the binding changed-path checkpoint rather than the pre-spawn one.
...
What it mechanically proves is bounded and its header says so: a reviewer id distinct from the task, a file present at that id's metadata path, and a review pinned to established content.
Neither dispatch provenance, independence, nor review activity is established mechanically - `reviewed` is a supervisor-owned assertion and those rest there.
